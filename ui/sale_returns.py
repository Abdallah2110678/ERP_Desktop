...
class NewSaleReturnDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("تسجيل مرتجع بيع")
        self.resize(820, 600)
    ...
    def _setup_ui(self):
        ...
        strip.setStyleSheet(f"background: {C_RETURN}; border-radius: 3px; border: none;")
        layout.addWidget(strip)

        title = QLabel("↩  تسجيل مرتجع بيع")
        title.setFont(QFont("Tahoma", 15, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {C_TEXT_DARK};")
        layout.addWidget(title)

        # Customer + invoice type row
        ...
        add_btn.setStyleSheet(f"""
            QPushButton {{ background: {C_RETURN}; color: white; border: none;
                padding: 8px 18px; border-radius: 7px; font-family: Tahoma; font-size: 13px; font-weight: bold; }}
            QPushButton:hover {{ background: {C_RETURN_DARK}; }}
        """)
        ...
        self.items_table.setHorizontalHeaderLabels(["حذف", "الإجمالي (ج.م)", "السعر (ج.م)", "الكمية", "الصنف"])
        ...
        self.total_label.setStyleSheet(f"color: {C_RETURN};")
    ...
    def _load_customers(self):
        ...
        for c in db.get_all_customers():
            self.customer_combo.addItem(
                f"{c['name']}   —   دين: {c['total_debt']:.2f} ج.م", c['id']
            )
    ...
    def _confirm(self):
        ...
        show_info(self, "تم بنجاح",
                  ...
                  "تم إضافة الأصناف للمخزون تلقائياً")
        self.accept()


class SaleReturnsPage(QWidget):
    def __init__(self):
        super().__init__()
        self.setLayoutDirection(Qt.LayoutDirection.RightToLeft)
    ...
    def _setup_ui(self):
        ...
        header_frame, header_inner = page_header(
            "↩  مرتجعات البيع",
            "سجّل المرتجعات التي أعادها العملاء"
        )
        new_btn = QPushButton("＋  تسجيل مرتجع بيع")
        ...
        self.table.setHorizontalHeaderLabels([
            "الإجراءات", "نوع الفاتورة", "الإجمالي (ج.م)", "العميل", "التاريخ"
        ])
    ...
    def load_returns(self):
        ...
        for row, r in enumerate(returns):
            ...
            cust_item = QTableWidgetItem(r.get('customer_name', '') or "—")
            cust_item.setFont(QFont("Tahoma", 11, QFont.Weight.Bold))
            cust_item.setForeground(QColor(C_TEXT_DARK))
            self.table.setItem(row, 3, cust_item)

            total_item = QTableWidgetItem(f"{r['total_amount']:.2f}")
            total_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            total_item.setForeground(QColor(C_RETURN))
